...
2. `saia-gwdg-plugin.js` — plugin's `config` hook runs at startup: installs a request pacer (wraps `globalThis.fetch` for the SAIA host: ≥2.1s between requests, one waited retry on 429, and multi-key failover — the pacer rewrites the `Authorization` header on every request to the currently active key, tracks each key's budget separately, and switches to the next usable key when the active one hits the floor (≤5 hourly / ≤10 daily / ≤30 monthly requests remain) or 429s despite pacing; an exhausted key re-enters rotation after its bucket's reset TTL (hour 60 min / day 24 h / month 30 d); hard abort with a clear error only when ALL keys are exhausted or after 3 consecutive 5xx responses; always writes per-key remaining-budget counts to `~/.cache/opencode/saia-gwdg-budget.json` after every response that carries rate-limit headers; `SAIA_PACER_DEBUG=1` logs each request to `~/.cache/opencode/saia-gwdg-pacer.log`), reads the API key from `~/.local/share/opencode/auth.json` plus optional extra failover keys from `~/.local/share/opencode/saia-gwdg-keys.json` (`{"keys": ["...", ...]}`, chmod 600; the auth.json key is always #1 in rotation; missing file = single-key behavior), loads the model list from the weekly cache (`~/.cache/opencode/saia-gwdg-models.json` is authoritative while its `fetchedAt` is <7 days old — most launches cost zero SAIA requests; older/missing cache triggers one `/v1/models` fetch, with the stale cache as failure fallback; force a refresh with `/reload_models` or `bash reload-models.sh`, then restart), injects only `status === "ready"` models into the provider (auto-detecting attachment/reasoning support), then **overwrites each agent's model** using the `ROLE_MODELS` preference table (first ready preference wins, else any ready model; the `auto` role additionally prefers reasoning-capable models). Finally it loads `prompts/auto.md` and `prompts/solo.md` itself and replaces the `__SAIA_BUDGET_STATUS__` placeholder with a LOW/HEALTHY/UNKNOWN budget status (hour/day/month counts) computed from the pacer's snapshot file. That prompt gate is advisory only (deepseek ignored it under task pressure) — the real enforcement is the plugin's `tool.execute.before` hook, which refuses the first `task` call of a session when the budget is LOW (<40 hour / <50 day / <60 month remaining, summed across all keys — a key without fresh snapshot data counts as full, a bucket the pacer stamped exhausted counts as empty; later task calls in the same session pass, so an in-flight chain is never cut off). Don't rename the placeholder or move the prompt files; agents must never try to read cache files themselves (reads outside the project are permission-blocked).
3. `~/.local/share/opencode/auth.json` (chmod 600, outside this repo) — holds the API key under the `saia-gwdg` key. If missing or unreadable, the plugin silently returns and no models load. Optional sibling `saia-gwdg-keys.json` holds extra failover keys; it is read only by the plugin (and `reload-models.sh`), never by opencode itself.
...
- **GWDG rate limits are tight and shared across everything**: one per-key bucket of 30 requests/min, 200/hour, 1000/day, 3000/month (see `x-ratelimit-*` response headers) covering chat completions AND `/v1/models`. Every agent step in an auto-mode run is one request. The plugin's pacer spaces requests so the per-minute limit can't trip, fails over to the next key in `saia-gwdg-keys.json` when the active key's bucket is nearly empty, and aborts with a clear error only when every key is drained — if a run dies with an "All N SAIA key(s) nearly exhausted" error, that's the pacer working; wait for the reset or add another key. Check budget with a cheap 1-token chat request and `curl -D -` (`/v1/models` does NOT return rate-limit headers), or `bash usage.sh` (free, reads the pacer's snapshot).
